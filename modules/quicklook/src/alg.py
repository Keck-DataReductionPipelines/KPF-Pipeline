...
class QuicklookAlg:
    ...
    def qlp_procedures(self,kpf0_file,output_dir,end_of_night_summary):
        ...
        if len(hdulist['EXPMETER_SCI'].data)>=1:
            ...
            lns2 = ax2.plot(wav_SCI, int_SCI_spec, marker='o', color='k', label ='SCI',zorder = 1)
            ax2 = ax2.twinx()
            lns1 = ax1.plot(wav_SKY, int_SKY_spec, marker='o', color='brown', label = 'SKY',zorder = 0)
            ax2.set_ylim(0,np.percentile(int_SCI_spec,99.9)*1.1)
            ax1.set_ylim(0,np.percentile(int_SKY_spec,99.9)*1.1)
            ax2.set_xlabel("Wavelength (nm)",fontsize=12)
            ax2.set_ylabel("SCI Exposure Meter Flux (e-/nm/s)",fontsize=12)
            ax1.set_ylabel("SKY Exposure Meter Flux (e-/nm/s)",fontsize=12)
