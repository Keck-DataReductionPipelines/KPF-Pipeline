...
class CCFStats(object):
    ...
    def clean_ccf(ccf, ccf_fit_results, width=1):
        ...
        leftpeak, rightpeak = sorted(crossings[np.argsort(np.abs(crossings - x0))[:2]])# leftpeak  = fsolve(diccf, x0-dx*fwhm)[0]
        
        # Want both wings to be at or pre-turnover for measurement
        continuum = min(iccf(leftpeak), iccf(rightpeak))

        # Normalize
        norm_ccf = ccf / continuum # Define 'continuum' == 1
        norm_iccf = interp1d(g_x, norm_ccf); ccfmin = norm_iccf(x0);
        norm_ccf = (norm_ccf - ccfmin) / (1 - ccfmin) # Set min to 0
        return norm_ccf
    ...
    def bisector(ccf, ccf_fit_results, ddepth=0.01, width=2, testing=False):
        ...
        # Cut out wings
        centered = (g_x >= x0 - width*fwhm) & (g_x <= x0 + width*fwhm)
        ccfx = g_x[centered]
        ccfy = norm_ccf[centered]
