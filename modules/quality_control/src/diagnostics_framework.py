...
import modules.quality_control.src.diagnostics as diatnostics
...
class DiagnosticsFramework(KPF0_Primitive):
    ...
    def __init__(self, action, context):
        ...
        self.logger.info('self.data_level_str = {}'.format(self.data_type_str))
    ...
    def _perform(self):
        ...
        if 'L0' in self.data_level_str:
            ...
            if self.diagnostics_name == 'write_read_noise':
                self.logger.info('Measuring diagnostics: {}'.format(self.diagnostics_name))
                self.kpf_object = diagnostics.write_read_noise(self.kpf_object, logger=None)
