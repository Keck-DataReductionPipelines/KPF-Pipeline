...
def main(start_date, end_date):
    ...
    base_dir = "/data"
    missing_L0 = []
    all_files = glob.glob(f"{base_dir}/L0/????????/*.fits")
    ...
    print(f"Number of files: {len(filtered_files)}")
    
    # to-do check if there are no matching files and stop if so
        
    strings = ' '.join(filtered_files)
    ncpus=10 # to-do - make this a parameter
    #command = f"parallel -j {ncpus} -v --bar kpf -c <(sed 's/INSERT_FITS_PATH/{{}}/' configs/quicklook_parallel.cfg) -r recipes/quicklook_match.recipe ::: {strings}"
    #command = f"bash -c \"parallel -j {ncpus} -v --ungroup kpf -c <(sed 's/INSERT_FITS_PATH/{{}}/' configs/quicklook_parallel.cfg) -r recipes/quicklook_match.recipe ::: {strings}\""
    #command = f"bash -c \"parallel -j {ncpus} -v --ungroup echo <(sed 's/INSERT_FITS_PATH/{{}}/' configs/quicklook_parallel.cfg) ::: {strings}\""

    #command = """parallel -j 10 -v --bar --ungroup bash -c 'echo "Starting Quicklook"; config=$(mktemp) && sed "s|INSERT_FITS_PATH|{}|" configs/quicklook_parallel.cfg > "$config" && kpf -c "$config" -r recipes/quicklook_match.recipe && rm "$config"' ::: {strings}"""
    #command = f"""parallel -j 10 -v --bar --ungroup bash -c 'echo "Starting Quicklook"; config=$(mktemp) && sed "s|INSERT_FITS_PATH|{{}}|" configs/quicklook_parallel.cfg > "$config" && kpf -c "$config" -r recipes/quicklook_match.recipe && rm "$config"' ::: {strings}"""
    command = f"""bash -c "parallel -j {ncpus} -k -v --bar --ungroup bash -c 'echo \\"Starting Quicklook instance {{}}\\"; config=\$(mktemp) && sed \\"s|INSERT_FITS_PATH|{{}}|\\" configs/quicklook_parallel.cfg > \\"\\$config\\" && kpf -c \\"\\$config\\" -r recipes/quicklook_match.recipe && rm \\"\\$config\\"' ::: {strings}" """
    ...
    try:
        ...
        # If there's a need to display output and errors after the command fails, consider logging them or handling them as needed.
