...
def add_headers_2D_flux_stats_in_out_ordertrace(D2, logger=None):
    ...
    if 'green' in chips:
        green_in    = output['green']['in']
        green_out   = output['green']['out']
        green_ratio = output['green']['ratio']
        D2.header['PRIMARY']['FINTRG9F'] = (float(green_in[6]), 'Flux [e-] inside padded order trace regions (Green, 99.5th %ile)')
        D2.header['PRIMARY']['FINTRG99'] = (float(green_in[5]), 'Flux [e-] inside padded order trace regions (Green, 99th %ile)')
        D2.header['PRIMARY']['FINTRG98'] = (float(green_in[4]), 'Flux [e-] inside padded order trace regions (Green, 98th %ile)')
        D2.header['PRIMARY']['FINTRG95'] = (float(green_in[3]), 'Flux [e-] inside padded order trace regions (Green, 95th %ile)')
        D2.header['PRIMARY']['FINTRG90'] = (float(green_in[2]), 'Flux [e-] inside padded order trace regions (Green, 90th %ile)')
        D2.header['PRIMARY']['FINTRG50'] = (float(green_in[1]), 'Flux [e-] inside padded order trace regions (Green, 50th %ile)')
        D2.header['PRIMARY']['FINTRG10'] = (float(green_in[0]), 'Flux [e-] inside padded order trace regions (Green, 10th %ile)')
        D2.header['PRIMARY']['FOUTRG9F'] = (float(green_out[6]), 'Flux [e-] outside padded order trace regions (Green, 99.5th %ile)')
        D2.header['PRIMARY']['FOUTRG99'] = (float(green_out[5]), 'Flux [e-] outside padded order trace regions (Green, 99th %ile)')
        D2.header['PRIMARY']['FOUTRG98'] = (float(green_out[4]), 'Flux [e-] outside padded order trace regions (Green, 98th %ile)')
        D2.header['PRIMARY']['FOUTRG95'] = (float(green_out[3]), 'Flux [e-] outside padded order trace regions (Green, 95th %ile)')
        D2.header['PRIMARY']['FOUTRG90'] = (float(green_out[2]), 'Flux [e-] outside padded order trace regions (Green, 90th %ile)')
        D2.header['PRIMARY']['FOUTRG50'] = (float(green_out[1]), 'Flux [e-] outside padded order trace regions (Green, 50th %ile)')
        D2.header['PRIMARY']['FOUTRG10'] = (float(green_out[0]), 'Flux [e-] outside padded order trace regions (Green, 10th %ile)')
        D2.header['PRIMARY']['FRATRG9F'] = (float(green_ratio[6]), 'Flux in/out padded order trace regions (Green, 99.5th %ile)')
        D2.header['PRIMARY']['FRATRG99'] = (float(green_ratio[5]), 'Flux in/out padded order trace regions (Green, 99th %ile)')
        D2.header['PRIMARY']['FRATRG98'] = (float(green_ratio[4]), 'Flux in/out padded order trace regions (Green, 98th %ile)')
        D2.header['PRIMARY']['FRATRG95'] = (float(green_ratio[3]), 'Flux in/out padded order trace regions (Green, 95th %ile)')
        D2.header['PRIMARY']['FRATRG90'] = (float(green_ratio[2]), 'Flux in/out padded order trace regions (Green, 90th %ile)')
        D2.header['PRIMARY']['FRATRG50'] = (float(green_ratio[1]), 'Flux in/out padded order trace regions (Green, 50th %ile)')
        D2.header['PRIMARY']['FRATRG10'] = (float(green_ratio[0]), 'Flux in/out padded order trace regions (Green, 10th %ile)')
    if 'red' in chips:
        red_in    = output['red']['in']
        red_out   = output['red']['out']
        red_ratio = output['red']['ratio']
        D2.header['PRIMARY']['FINTRR9F'] = (float(red_in[6]), 'Flux [e-] inside padded order trace regions (Red, 99.5th %ile)')
        D2.header['PRIMARY']['FINTRR99'] = (float(red_in[5]), 'Flux [e-] inside padded order trace regions (Red, 99th %ile)')
        D2.header['PRIMARY']['FINTRR98'] = (float(red_in[4]), 'Flux [e-] inside padded order trace regions (Red, 98th %ile)')
        D2.header['PRIMARY']['FINTRR95'] = (float(red_in[3]), 'Flux [e-] inside padded order trace regions (Red, 95th %ile)')
        D2.header['PRIMARY']['FINTRR90'] = (float(red_in[2]), 'Flux [e-] inside padded order trace regions (Red, 90th %ile)')
        D2.header['PRIMARY']['FINTRR50'] = (float(red_in[1]), 'Flux [e-] inside padded order trace regions (Red, 50th %ile)')
        D2.header['PRIMARY']['FINTRR10'] = (float(red_in[0]), 'Flux [e-] inside padded order trace regions (Red, 10th %ile)')
        D2.header['PRIMARY']['FOUTRR9F'] = (float(red_out[6]), 'Flux [e-] outside padded order trace regions (Red, 99.5th %ile)')
        D2.header['PRIMARY']['FOUTRR99'] = (float(red_out[5]), 'Flux [e-] outside padded order trace regions (Red, 99th %ile)')
        D2.header['PRIMARY']['FOUTRR98'] = (float(red_out[4]), 'Flux [e-] outside padded order trace regions (Red, 98th %ile)')
        D2.header['PRIMARY']['FOUTRR95'] = (float(red_out[3]), 'Flux [e-] outside padded order trace regions (Red, 95th %ile)')
        D2.header['PRIMARY']['FOUTRR90'] = (float(red_out[2]), 'Flux [e-] outside padded order trace regions (Red, 90th %ile)')
        D2.header['PRIMARY']['FOUTRR50'] = (float(red_out[1]), 'Flux [e-] outside padded order trace regions (Red, 50th %ile)')
        D2.header['PRIMARY']['FOUTRR10'] = (float(red_out[0]), 'Flux [e-] outside padded order trace regions (Red, 10th %ile)')
        D2.header['PRIMARY']['FRATRR9F'] = (float(red_ratio[6]), 'Flux in/out padded order trace regions (Red, 99.5th %ile)')
        D2.header['PRIMARY']['FRATRR99'] = (float(red_ratio[5]), 'Flux in/out padded order trace regions (Red, 99th %ile)')
        D2.header['PRIMARY']['FRATRR98'] = (float(red_ratio[4]), 'Flux in/out padded order trace regions (Red, 98th %ile)')
        D2.header['PRIMARY']['FRATRR95'] = (float(red_ratio[3]), 'Flux in/out padded order trace regions (Red, 95th %ile)')
        D2.header['PRIMARY']['FRATRR90'] = (float(red_ratio[2]), 'Flux in/out padded order trace regions (Red, 90th %ile)')
        D2.header['PRIMARY']['FRATRR50'] = (float(red_ratio[1]), 'Flux in/out padded order trace regions (Red, 50th %ile)')
        D2.header['PRIMARY']['FRATRR10'] = (float(red_ratio[0]), 'Flux in/out padded order trace regions (Red, 10th %ile)')
