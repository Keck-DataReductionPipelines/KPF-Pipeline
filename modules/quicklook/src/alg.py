...
class QuicklookAlg:
    ...
    def qlp_procedures(self,kpf0_file,output_dir,end_of_night_summary):
        ...
        if os.path.exists(L1_data):
            ...
            plt.plot(np.nanmedian(wav,axis = 1),np.nanmedian(flux2/flux,axis = 1), label = 'Sci2/Sci1')
            plt.plot(np.nanmedian(wav,axis = 1),np.nanmedian(flux3/flux,axis = 1), label = 'Sci3/Sci1')
            plt.plot(np.nanmedian(wav,axis = 1),np.nanmedian(flux_sky/flux,axis = 1), label = 'Cal/Sci1')
            plt.plot(np.nanmedian(wav,axis = 1),np.nanmedian(flux_cal/flux,axis = 1), label = 'Sky/Sci1')
            plt.legend()
