...
class QuicklookAlg:
    ...
    def qlp_procedures(self,kpf0_file,output_dir,end_of_night_summary):
        ...
        if os.path.exists(L1_data):
            ...
            plt.figure(figsize=(10,4))
            plt.subplots_adjust(left=0.1, bottom=0.15, right=0.9, top=0.9)
            print('test orderlets', np.shape(flux2),np.shape(flux),np.nanmedian(flux2/flux,axis = 1),np.nanmedian(wav,axis = 1))
            plt.plot(np.nanmedian(wav_green,axis = 1),np.nanmedian(flux_green2/flux_green,axis = 1),marker = 'o', color = 'green', label = 'Sci2/Sci1')
            plt.plot(np.nanmedian(wav_green,axis = 1),np.nanmedian(flux_green3/flux_green,axis = 1),marker = 'o', color = 'red', label = 'Sci3/Sci1')
            plt.plot(np.nanmedian(wav_green,axis = 1),np.nanmedian(flux_green_cal/flux_green,axis = 1),marker = 'o', color = 'blue', label = 'Cal/Sci1')
            plt.plot(np.nanmedian(wav_green,axis = 1),np.nanmedian(flux_green_sky/flux_green,axis = 1),marker = 'o', color = 'magenta', label = 'Sky/Sci1')

            plt.plot(np.nanmedian(wav_red,axis = 1),np.nanmedian(flux_red2/flux_red,axis = 1),marker = 'D', facecolors='none', color = 'green')
            plt.plot(np.nanmedian(wav_red,axis = 1),np.nanmedian(flux_red3/flux_red,axis = 1),marker = 'D', facecolors='none', color = 'red')
            plt.plot(np.nanmedian(wav_red,axis = 1),np.nanmedian(flux_red_cal/flux_red,axis = 1),marker = 'D', facecolors='none', color = 'blue')
            plt.plot(np.nanmedian(wav_red,axis = 1),np.nanmedian(flux_red_sky/flux_red,axis = 1),marker = 'D', facecolors='none', color = 'magenta')
            plt.legend()
