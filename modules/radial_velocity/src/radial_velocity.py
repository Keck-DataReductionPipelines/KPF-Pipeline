...
class RadialVelocity(KPF1_Primitive):
    ...
    orderlet_key_map = {
        "kpf": {
            "sci_flux1": {
                "ccf": {"mask": "sci_mask", "name": "ccf1"},
                "rv": {"rv": "rv1", "erv": "erv1", "ccf": "ccf1"},
            },
            "sci_flux2": {
                "ccf": {"mask": "sci_mask", "name": "ccf2"},
                "rv": {"rv": "rv2", "erv": "erv2", "ccf": "ccf2"},
            },
            "sci_flux3": {
                "ccf": {"mask": "sci_mask", "name": "ccf3"},
                "rv": {"rv": "rv3", "erv": "erv3", "ccf": "ccf3"},
            },
            "sky_flux": {
                "ccf": {"mask": "sky_mask", "name": "ccf5"},
                "rv": {"rv": "rvs", "erv": "ervs", "ccf": "ccfs"},
            },
            "cal_flux": {
                "ccf": {"mask": "cal_mask", "name": "ccf4"},
                "rv": {"rv": "rvc", "erv": "ervc", "ccf": "ccfc"},
            },
        },
        "neid": {
            "sci": {
    ...
    orderlet_rv_col_map = {
        "kpf": {
            ...
            "sky_flux": {
                "orderlet": RV_COL_SKY,
                "source": RV_COL_SKY_SOURCE,
                "erv": RV_COL_SKY_ERR,
            },
            "cal_flux": {
                "orderlet": RV_COL_CAL,
                "source": RV_COL_CAL_SOURCE,
                "erv": RV_COL_CAL_ERR,
            },
        },
        "neid": {"sci": {"orderlet": "orderlet1", "source": "source1", "erv": ""}},
    }
    ...
    def __init__(self, action: Action, context: ProcessingContext) -> None:
        ...
        self.od_names = (
            action.args[3] if isinstance(action.args[3], list) else [action.args[3]]
        )
        self.ref_ccf = None

        if "input_ref" in args_keys:
            if isinstance(action.args["input_ref"], np.ndarray) or isinstance(
                action.args["input_ref"], pd.DataFrame
            ):
                self.ref_ccf = action.args["input_ref"]
            elif isinstance(action.args["input_ref"], str) and os.path.exists(
                action.args["input_ref"]
            ):
                self.ref_ccf = pd.read_csv(action.args["input_ref"], sep="\s+")

        self.ccf_engine = (
            action.args["ccf_engine"].lower()
        ...
        self.segment_limits = (
            self.load_csv(action.args["segment_def"])
            if "segment_def" in args_keys
            else None
        )
        self.order_limits = (
            self.load_csv(action.args["order_def"])
            if "order_def" in args_keys
            else None
        )
        ...
        self.reweighting_method = (
            action.args["reweighting_method"]
            if "reweighting_method" in args_keys
            else None
        )
        self.ccf_ext = (
            action.args["ccf_ext"]
            if "ccf_ext" in args_keys
            else self.default_args_val["ccf_ext"]
        )
        self.rv_ext = (
            action.args["rv_ext"]
            if "rv_ext" in args_keys
            else self.default_args_val["rv_ext"]
        )
        self.rv_set_idx = (
            action.args["rv_set"]
            if "rv_set" in args_keys
            else self.default_args_val["rv_set"]
        )
        self.is_cal_cor = (
            action.args["rv_correction_by_cal"]
            if "rv_correction_by_cal" in args_keys
            else False
        )

        bary_corr_ext = (
            action.args["bary_corr"]
            if "bary_corr" in args_keys
            else self.default_args_val["bary_corr"]
        )
        bc_table = (
            getattr(self.input, bary_corr_ext)
            if hasattr(self.input, bary_corr_ext)
            else None
        )
        start_bary_index = (
            action.args["start_bary_index"]
            if "start_bary_index" in args_keys
            else self.default_args_val["start_bary_index"]
        )
        ...
        self.reweighting_masks = (
            action.args["reweighting_masks"]
        ...
        exptime_v = (
            action.args["exptime"]
            if "exptime" in args_keys
            else self.default_args_val["exptime"]
        )
        obstime_v = (
            action.args["obstime"]
            if "obstime" in args_keys
            else self.default_args_val["obstime"]
        )
        self.sci_names = list()
        for sci in self.od_names:
            input_data = (
                getattr(self.input, sci)
                if self.input is not None and hasattr(self.input, sci)
                else None
            )
            input_header = (
                self.input.header[sci]
                if input_data is not None and hasattr(self.input, "header")
                else None
            )
            ...
            self.wave_cal_set.append(
                getattr(self.input, wave)
                if (
                    self.input is not None
                    and wave is not None
                    and hasattr(self.input, wave)
                )
                else None
            )
            ...
            if not self.rv_init["data"][mod]:
                ...
                self.input.header[sci]["MASK"] = self.rv_init["data"][mod][
                    RadialVelocityAlg.get_fiber_object_in_header(self.ins, sci)
                ][mtype]
        ...
        if p_header is not None:
            ...
            if self.rv_init["data"][RadialVelocityAlgInit.MASK_ORDERLET]:
                cal_mask = self.rv_init["data"][RadialVelocityAlgInit.MASK_ORDERLET][
                    key_cal
                ][RadialVelocityAlgInit.MASK_TYPE]
                do_rv_corr = (
                    cal_mask in ["lfc", "thar", "etalon"]
                    and sci_obj_v.lower() == "target"
                )

            self.is_solar_data = (
                "OBJECT" in p_header
                and p_header["OBJECT"].lower() == "sun"
                and key_sci in p_header
                and p_header[key_sci].lower() == "target"
                and key_sky in p_header
                and p_header[key_sky].lower() == "target"
            )
        self.is_cal_cor = self.is_cal_cor and do_rv_corr
        self.mask_collection = []

        if self.rv_init["data"][RadialVelocityAlgInit.MASK_ORDERLET]:
            for mt_key in self.rv_init["data"][
                RadialVelocityAlgInit.MASK_ORDERLET
            ].keys():
                m_type = self.rv_init["data"][RadialVelocityAlgInit.MASK_ORDERLET][
                    mt_key
                ][RadialVelocityAlgInit.MASK_TYPE]
                if (
                    m_type in RadialVelocityAlg.vel_range_per_mask
                    and m_type not in self.mask_collection
                ):
                    self.mask_collection.append(m_type)

        vel_span_pixel = (
            action.args["vel_span_pixel"] if "vel_span_pixel" in args_keys else None
        )
        self.reweighted = {}
        ...
        try:
            if self.ins is None:
                ...
                self.alg = RadialVelocityAlg(
                    self.spectrum_data_set[0],
                    self.header_set[0],
                    self.rv_init,
                    ...
                    config=self.config,
                    logger=self.logger,
                    ccf_engine=self.ccf_engine,
                    reweighting_method=self.reweighting_method,
                    bary_corr_table=bc_table,
                    start_bary_index=start_bary_index,
                    orderlet=self.od_names[0],
    ...
    def get_ratio_ccf(self, od_name):
        ...
        def is_mask_enable(mask):
            return mask is not None and (
                self.reweighting_masks is None
                or any([m.lower() in mask.lower() for m in self.reweighting_masks])
            )
        ...
        if self.ref_ccf is not None:
            if self.reweighting_method.lower() == "ccf_static":
                ...
                if is_mask_enable(m_type):
                    idx = [c.lower() for c in self.ref_ccf.columns].index(
                        m_type.lower()
                    )
                    if idx >= 0:
                ...
                ratio_ccf = (
                    self.ref_ccf.values
                    if isinstance(self.ref_ccf, pd.DataFrame)
                    else self.ref_ccf
                )
        return ratio_ccf
    ...
    def _perform(self):
        ...
        if self.logger:
            self.logger.info(
                "RadialVelocity: Start crorss correlation to find radial velocity... "
            )

        if self.alg is None:
            self.logger.info(
                "RadialVelocity: no enough data to start the instance to do cross correlation... "
            )
        ...
        if all([s is not None and s.size != 0 for s in self.spectrum_data_set]):
            for i in range(self.total_orderlet):
                if i > 0:
                    self.alg.reset_spectrum(
                        self.spectrum_data_set[i],
                        self.header_set[i],
                        self.wave_cal_set[i],
                ...
                if self.logger:
                    self.logger.info(
                        "RadialVelocity: computing radial velocity on orderlet "
                        + self.od_names[i]
                        + "..."
                    )

                ratio_ccf = self.get_ratio_ccf(self.od_names[i])
                self.reweighted[self.od_names[i]] = (
                    "T" if ratio_ccf is not None else "F"
                )
                rv_results = self.alg.compute_rv_by_cc(
                    start_seg=self.start_seg, end_seg=self.end_seg, ref_ccf=ratio_ccf
                )
                one_df = rv_results["ccf_df"]
                if one_df is None or one_df.empty or not one_df.values.any():
                    if self.logger:
                        self.logger.info(
                            "RadialVelocity: orderlet "
                            + self.od_names[i]
                            + " message => "
                            + rv_results["msg"]
                            or "all ccfs are zeros"
                        )
                    one_df = None
        ...
        self.output_level2.receipt_add_entry(
            "RadialVelocity", self.__module__, f"config_path={self.config_path}", "PASS"
        )
    ...
    def construct_level2_data(self, output_df):
        ...
        self.output_level2.header[self.rv_ext]["star_rv"] = new_rv_table.attrs[
            "star_rv"
        ]
        ...
        for od_name in self.od_names:
            ...
            if "rw" + ccf_key not in self.output_level2.header[self.rv_ext]:
                self.output_level2.header[self.rv_ext]["rw" + ccf_key] = (
                    self.reweighted[od_name]
                )
        ...
        for m in RadialVelocityAlg.vel_range_per_mask.keys():
            if m in self.mask_collection:
                self.add_ext_key(
                    self.output_level2.header[self.rv_ext],
                    "vr_" + m,
                    RadialVelocityAlg.vel_range_per_mask[m],
                )

        crt_rv_ext = (
            self.output_level2[self.rv_ext]
            if hasattr(self.output_level2, self.rv_ext)
            else None
        )
        if crt_rv_ext is None or np.shape(crt_rv_ext)[0] == 0:
            self.output_level2[self.rv_ext] = new_rv_table
            self.output_level2.header[self.rv_ext][
                "ccd" + str(self.rv_set_idx + 1) + "row"
            ] = 0
        else:
            ...
            for c_name in self.rv_col_names:
                if c_name in self.rv_col_on_orderlet:
                    for od_name in self.sci_names:
                        c_name_orderlet = self.orderlet_rv_col_map[self.ins][
                            self.get_map_key(od_name)
                        ][c_name]
                        new_list = (
                            crt_rv_ext[c_name_orderlet].tolist()
                            + new_rv_table[c_name_orderlet].tolist()
                        )
                        new_table_list[c_name_orderlet] = new_list
                else:
                    if c_name in crt_rv_ext and c_name in new_rv_table:
                        new_list = (
                            crt_rv_ext[c_name].tolist() + new_rv_table[c_name].tolist()
                        )
        ...
        for od_name in self.od_names:
            ...
            self.output_level2.header[self.rv_ext][ccd_p + rv_key] = new_rv_table.attrs[
                ccd_ + rv_key
            ]
            self.output_level2.header[self.rv_ext][ccd_p + erv_key] = (
                new_rv_table.attrs[ccd_ + erv_key]
            )

        self.output_level2.header[self.rv_ext][ccd_p + "rv"] = new_rv_table.attrs[
            ccd_ + "rv"
        ]
        self.output_level2.header[self.rv_ext][ccd_p + "erv"] = new_rv_table.attrs[
            ccd_ + "erv"
        ]
        # self.output_level2.header[self.rv_ext][ccd_p+'jd'] = new_rv_table.attrs[ccd_+'jd']
        self.output_level2.header[self.rv_ext][ccd_p + "bjd"] = new_rv_table.attrs[
            ccd_ + "bjd"
        ]
        self.output_level2.header[self.rv_ext][
            "rv" + str(self.rv_set_idx + 1) + "corr"
        ] = ("T" if new_rv_table.attrs["do_rv_corr"] else "F")
    ...
    def make_ccf_table(self, output_df):
        ...
        for i in range(
            total_orderlet
        ):  # make ccf in 3d format, each ccf layer is for one SCI orderlet
            if output_df[self.od_names[i]] is not None:
                reweight_ccf[i, :, :] = output_df[self.od_names[i]].values[
                    0:total_segment, :
                ]
                if self.reweighted[self.od_names[i]] == "T":
                    original_ccf[i, :, :] = output_df[self.od_names[i]].attrs[
                        "ORI_CCF"
                    ][0:total_segment, :]
                else:
                    original_ccf[i, :, :] = output_df[self.od_names[i]].values[
                        0:total_segment, :
                    ]
        ...
        if self.ins.lower() == "kpf":
            ...
            if (
                len(ccf_exts) > 1
            ):  # only assign reweighted ccf if reweighting is done on some orderlet
        ...
        for ext in ccf_exts:
            ...
            self.output_level2.header[ext]["startv"] = (
                self.rv_init["data"][RadialVelocityAlgInit.VELOCITY_LOOP][0],
                "km/sec",
            )
            self.output_level2.header[ext]["stepv"] = (
                self.rv_init["data"]["rv_config"][RadialVelocityAlgInit.STEP],
                "km/sec",
            )
            self.output_level2.header[ext]["totalv"] = self.rv_init["data"][
                "velocity_steps"
            ]
        ...
        for i in range(total_orderlet):
            ...
            for ext in ccf_exts:
                self.output_level2.header[ext][
                    self.orderlet_key_map[self.ins][map_key]["ccf"]["name"]
                ] = self.od_names[i]
            ...
            for ext in ccf_exts:
                if ccf_key not in self.output_level2.header[ext]:
                    self.output_level2.header[ext][ccf_key] = (
                        RadialVelocityAlg.get_orderlet_masktype(
                            self.ins, self.od_names[i], self.rv_init["data"]
                        )
                    )
                    self.output_level2.header[ext][
                        ccf_key.replace("_mask", "mpath")
                    ] = RadialVelocityAlg.get_orderlet_maskpath(
                        self.ins, self.od_names[i], self.rv_init["data"]
                    )
    ...
    def make_rv_table(self, output_df):
        ...
        for idx, od_name in enumerate(self.od_names):
            ...
            if map_key:
                orderlet_cols[od_name] = self.orderlet_rv_col_map[self.ins][map_key][
                    "orderlet"
                ]
                source_cols[od_name] = self.orderlet_rv_col_map[self.ins][map_key][
                    "source"
                ]
                ...
                if source_cols[od_name]:
                    rv_table[source_cols[od_name]] = np.array(
                        [od_name] * total_segment, dtype=object
                    )
            else:
        ...
        for idx, od_name in enumerate(self.od_names):
            ...
            if (
                output_df[od_name] is None
            ):  # skip no ccf and no cal correction if some sci ccf is none
                if b_sci:
                    do_corr = False
            ...
            if sci_mask is None and b_sci:
                sci_mask = RadialVelocityAlg.get_orderlet_masktype(
                    ins, od_name, self.rv_init["data"]
                )
            ...
            orderlet_rvs[od_name] = output_df[od_name].attrs["CCF-RVC"][
                0
            ]  # method2: output_df[od_name].attrs['RV_MEAN']
            orderlet_ervs[od_name] = output_df[od_name].attrs[
                "CCF-ERV"
            ]  # method2: output_df[od_name].attrs['ERV_MEAN']
            ...
            if b_sci or (self.is_sky(od_name) and self.is_solar_data):
                ccf_summ_h_rw[self.start_seg : self.end_seg + 1, :] += output_df[
                    od_name
                ].values[self.start_seg : self.end_seg + 1, :]
                ccf_summ_h_rw_nonnorm[
                    self.start_seg : self.end_seg + 1, :
                ] += output_df[od_name].attrs["CCF_NONNORM"][
                    self.start_seg : self.end_seg + 1, :
                ]
                ...
                cal_rv = output_df[od_name].attrs["CCF-RVC"][
                    0
                ]  # record rv per segments and final rv for cal orderlet
        ...
        for r in range(self.start_seg, self.end_seg + 1):
            _, col_rv[r], _, _, col_rv_err[r] = RadialVelocityAlg.fit_ccf(
                ccf_summ_h_rw[r, :],
                self.alg.get_rv_guess(),
                self.rv_init["data"][RadialVelocityAlgInit.VELOCITY_LOOP],
                sci_mask,
                # self.input.header[od_name]['MASK'],
                rv_guess_on_ccf=(self.ins.lower() == "kpf"),
                vel_span_pixel=(
                    self.alg.get_vel_span_pixel() if sci_ccf_ratio is None else None
                ),
            )
            if sci_ccf_ratio is not None:
                _, _, _, _, col_rv_err[r] = RadialVelocityAlg.fit_ccf(
                    ccf_summ_h_rw_nonnorm[r, :],
                    self.alg.get_rv_guess(),
                    self.rv_init["data"][RadialVelocityAlgInit.VELOCITY_LOOP],
                    sci_mask,
                    # self.input.header[od_name]['MASK'],
        ...
        s_seg = 0
        e_seg = total_segment
        rv_table[self.RV_COL_START_W] = segment_table[
            s_seg:e_seg, RadialVelocityAlg.SEGMENT_W1
        ]
        rv_table[self.RV_COL_END_W] = segment_table[
            s_seg:e_seg, RadialVelocityAlg.SEGMENT_W2
        ]
        rv_table[self.RV_COL_SEG_NO] = segment_table[
            s_seg:e_seg, RadialVelocityAlg.SEGMENT_IDX
        ].astype(int)
        rv_table[self.RV_COL_ORD_NO] = segment_table[
            s_seg:e_seg, RadialVelocityAlg.SEGMENT_ORD
        ].astype(int)
        ...
        rv_table[self.RV_WEIGHTS] = (
            np.ones(total_segment)
            if sci_ccf_ratio is None
            else sci_ccf_ratio[:, -1][0:total_segment]
        )
        ...
        for od_name in self.od_names:
            ...
            if map_key:
                od_rv = (
                    orderlet_rvs[od_name] - cal_rv
                    if do_corr and self.is_sci(od_name)
                    else orderlet_rvs[od_name]
                )
                unit = self.get_rv_unit(od_name)
                results.attrs[
                    "ccd_" + self.orderlet_key_map[self.ins][map_key]["rv"]["rv"]
                ] = (f_decimal(od_rv), unit)
                results.attrs[
                    "ccd_" + self.orderlet_key_map[self.ins][map_key]["rv"]["erv"]
                ] = f_decimal(orderlet_ervs[od_name])

        final_rv = final_rv_err = 0.0

        # compute rv & rv error for the chip
        if sci_mask is not None:
            _, final_rv, _, _, final_rv_err = RadialVelocityAlg.fit_ccf(
                ccf_summ_v_rw,
                self.alg.get_rv_guess(),
                self.rv_init["data"][RadialVelocityAlgInit.VELOCITY_LOOP],
                sci_mask,
            ...
            if sci_ccf_ratio is not None:
                _, _, _, _, final_rv_err = RadialVelocityAlg.fit_ccf(
                    ccf_summ_v_rw_nonnorm,
                    self.alg.get_rv_guess(),
                    self.rv_init["data"][RadialVelocityAlgInit.VELOCITY_LOOP],
                    sci_mask,
                    rv_guess_on_ccf=(self.ins.lower() == "kpf"),
        ...
        results.attrs["ccd_rv"] = (
            (f_decimal(final_rv - cal_rv), unit)
            if do_corr and final_rv != 0.0
            else (f_decimal(final_rv), unit)
        )
    ...
    def full_instrument_rv(self):
        ...
        if rvh["CCD1ERV"] > 0 and rvh["CCD2ERV"] > 0:
            final_rv = (rvg * np.nansum(weights_green) / weights_all_ords) + (
                rvr * np.nansum(weights_red) / weights_all_ords
            )
            final_rv_err = 1 / np.sqrt(
                1 / rvh["CCD1ERV"] ** 2 + 1 / rvh["CCD2ERV"] ** 2
            )
        else:
            final_rv = 0
            final_rv_err = 0
        ...
        if rvh["CCD1ERVC"] > 0 and rvh["CCD2ERVC"] > 0:
            ...
            final_rv_err = 1 / np.sqrt(
                1 / rvh["CCD1ERVC"] ** 2 + 1 / rvh["CCD2ERVC"] ** 2
            )
        else:
            final_rv = 0
            final_rv_err = 0
    ...
    def load_csv(filepath, header=None):
        if filepath is not None and os.path.isfile(filepath):
            df = pd.read_csv(
                filepath, header=header, sep="\s+|\t+|\s+\t+|\t+\s+", engine="python"
            )
            return df.values
        else:
            return None
