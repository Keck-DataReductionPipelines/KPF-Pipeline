...
class QuicklookAlg:
    ...
    def qlp_procedures(self,kpf0_file,output_dir):
        ...
        for i_color in range(len(ccd_color)):
            ...
            for i in [50]:#range(np.shape(order_trace)[0])
                ...
                plt.fill_between(x_grid,y_grid-order_trace.iloc[i]['BottomEdge'],y_grid+order_trace.iloc[i]['TopEdge'],color ='pink',alpha = 0.5)
                print(x_grid,y_grid)
