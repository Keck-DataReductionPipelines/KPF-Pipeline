...
from modules.barycentric_correction.src.alg_barycentric_corr import (
    BarycentricCorrectionAlg,
)
...
class RadialVelocityAlg(RadialVelocityBase):
    ...
    def __init__(
        ...
        if (
            area_limits is not None
            and not isinstance(area_limits, list)
            and len(area_limits) != 4
        ):
        ...
        self.velocity_loop = init_data[
            RadialVelocityAlgInit.VELOCITY_LOOP
        ]  # loop of velocities for rv finding
        self.velocity_steps = init_data[
            RadialVelocityAlgInit.VELOCITY_STEPS
        ]  # total steps in velocity_loop
        self.mask_line = init_data[RadialVelocityAlgInit.MASK_LINE]  # def_mask
        self.reweighting_ccf_method = (
            init_data[RadialVelocityAlgInit.REWEIGHTING_CCF]
            if reweighting_method is None
            or not self.is_good_reweighting_method(reweighting_method)
            else reweighting_method
        )
        self.ccf_code = (
            ccf_engine
            if (ccf_engine and ccf_engine in ["c", "python"])
            else init_data[RadialVelocityAlgInit.CCF_CODE]
        )
        ...
        self.end_x_pos = nx - 1
        self.spectro = (
            self.rv_config[RadialVelocityAlgInit.SPEC].lower()
            if RadialVelocityAlgInit.SPEC in self.rv_config
            else "neid"
        )
        ...
        self.vel_span_pixel = (
            vel_span_pixel
            if vel_span_pixel is not None
            else RadialVelocityAlg.comp_velocity_span_pixel(
                init_data, config, self.spectro
            )
        )
        self.obs_bjd = None
    ...
    def comp_velocity_span_pixel(init_data, config, instrument):
        if init_data is not None:
            ...
            vel_span_pixel = RadialVelocityAlg.get_config_value(
                config, instrument, RadialVelocityAlgInit.VEL_SPAN_PIXEL, None
            )
    ...
    def get_segment_limits(self, seg_idx=0):
        if self.segment_limits_table is None:
            segment_list = (
                []
            )  # segment_index,  start_x, end_x, start_wavelength, end_wavelength, order_index
            ...
            if self.segment_limits is not None:  # per what segment limits define
                ...
                for s in range(seg_total):
                    ...
                    sel_w = np.where(
                        (self.wave_cal >= wlen[0]) & (self.wave_cal <= wlen[1])
                    )
                    if np.size(sel_w[0]) > 0:
                        sel_order = (
                            self.segment_limits[s, -1]
                            if self.segment_limits[s, -1] in sel_w[0]
                            else sel_w[0][0]
                        )
                ...
                seg_list = np.arange(
                    0, self.end_order + 1, dtype=int
                )  # list with full orders
                order_list = self.order_limits_mask[
                    :, 0
                ]  # order_index in order limits mask
                for ord_idx in range(seg_list):

                    if ord_idx in order_list:
                        r = np.where(order_list == ord_idx)[0][0]
                        limits = [
                            self.order_limits_mask[r, 1],
                            self.end_x_pos - self.order_limits_mask[r, num_limits],
                        ]
                    else:
                    ...
                    segment_list.append(
                        [
                            ...
                            self.wave_cal[ord_idx, limits[1]],
                            ord_idx,
                        ]
                    )
            elif (
                self.area_limits is not None
            ):  # all orders are counted, 1-1 between segments and orders
                order_range = [
                    (
                        self.area_limits[i]
                        if self.area_limits[i] >= 0
                        else self.end_order + self.area_limits[i]
                    )
                    for i in [0, 1]
                ]
                    ...
                    if order_range[0] <= r <= order_range[1]:
                        x_range = [
                            (
                                self.area_limits[i]
                                if self.area_limits[i] >= 0
                                else (self.end_x_pos + self.area_limits[i])
                            )
                            for i in [2, 3]
                        ]
                        segment_list.append(
                ...
                for r in range(self.start_order, self.end_order + 1):
                    segment_list.append(
                        [
                            ...
                            self.wave_cal[r, self.end_x_pos],
                            r,
                        ]
                    )
        ...
        if self.obs_jd is None:
            ...
            if self.spectro == "neid":
                ...
                for s in range(self.get_total_segments()):
                    self.obs_jd[s], self.obs_bjd[s] = self.get_obs_time_kpf(
                        default=default, seg=s
                    )
    ...
    def get_obs_time_harps(self, default=None):
        if "MJD-OBS" in self.header and "EXPTIME" in self.header:
            obs_time = (
                self.header["MJD-OBS"]
                + 2400000.5
                + self.header["EXPTIME"] * SEC_TO_JD / 2
            )
        else:
            obs_time = default
    ...
    def get_obs_time_kpf(self, default=None, seg=0):
        ...
        if (
            seg >= 0
            and self.bary_corr_table is not None
            and not self.bary_corr_table.empty
            and np.shape(self.bary_corr_table.values)[0] > ord_idx
        ):
            obs_time = np.array(self.bary_corr_table["PHOTON_JD"])[
                ord_idx + self.start_bary_index
            ]
            obs_bjd = np.array(self.bary_corr_table["PHOTON_BJD"])[
                ord_idx + self.start_bary_index
            ]
    ...
    def get_redshift_kpf(self, seg=0, default=None):
        ...
        if (
            "MASK" in self.header
            and self.header["MASK"] in ["lfc", "thar", "etalon"]
            ...
            self.bary_corr_table is not None
            and np.shape(self.bary_corr_table.values)[0] > ord_idx
        ):
    ...
    def get_redshift_gen(self, default=None, seg=0):
        ...
        if obs_time is None:
            ...
            rv_config_bc_key = [
                RadialVelocityAlgInit.RA,
                RadialVelocityAlgInit.DEC,
                RadialVelocityAlgInit.PMRA,
                RadialVelocityAlgInit.PMDEC,
                RadialVelocityAlgInit.EPOCH,
                RadialVelocityAlgInit.PARALLAX,
                RadialVelocityAlgInit.OBSLAT,
                RadialVelocityAlgInit.OBSLON,
                RadialVelocityAlgInit.OBSALT,
                RadialVelocityAlgInit.STAR_RV,
                RadialVelocityAlgInit.SPEC,
                RadialVelocityAlgInit.STARNAME,
            ]
            rv_config_bc = {k: self.rv_config[k] for k in rv_config_bc_key}
            if RadialVelocityAlgInit.is_unknown_target(
                self.spectro,
                rv_config_bc[RadialVelocityAlgInit.STARNAME],
    ...
    def wavelength_calibration_neid(self, spectrum_x: np.ndarray):
        new_calibs = self.wave_cal[
            self.start_order : self.start_order + self.spectrum_order, spectrum_x
        ]
    ...
    def wavelength_calibration_harps(self, spectrum_x: np.ndarray):
        ...
        for ord_idx in range(self.start_order, self.spectrum_order + self.start_order):
            ...
            ll_coeffs_order = [
                calib_hdr[CAL_TH_COEFF + str(coeff_base + i)]
                for i in range(p_degree, -1, -1)
            ]
        ...
        for ord_idx in range(self.spectrum_order):
            wave_cals[ord_idx, :] = np.polyval(
                np.poly1d(calib_coeffs_orders[ord_idx]), spectrum_x
            )
    ...
    def get_rv_on_spectrum(self, start_seg=None, end_seg=None):
        ...
        wavecal_all_orders = self.wavelength_calibration(
            spectrum_x
        )  # from s_order to e_order, s_x to e_x
        ...
        for idx, seg_idx in np.ndenumerate(seg_ary):
            ...
            self.d_print(
                ...
                " ",
                [
                    int(seg_limits[self.SEGMENT_X1]),
                    int(seg_limits[self.SEGMENT_X2]),
                    seg_limits[self.SEGMENT_W1],
                    seg_limits[self.SEGMENT_W2],
                    int(seg_limits[self.SEGMENT_ORD]),
                ],
                " ",
            )
            ...
            if np.any(wavecal != 0.0):
                if wavecal[-1] < wavecal[0]:
                    ordered_spec = self.fix_nan_spectrum(
                        np.flip(new_spectrum[ord_idx][left_x:right_x])
                    )  # check??
                    ordered_wavecal = np.flip(wavecal[left_x:right_x])
                else:
                    ordered_spec = self.fix_nan_spectrum(
                        new_spectrum[ord_idx][left_x:right_x]
                    )
                    ordered_wavecal = wavecal[left_x:right_x]
                zb = self.get_redshift(seg=seg_idx)
                result_ccf[seg_idx, :] = self.cross_correlate_by_mask_shift(
                    ordered_wavecal, ordered_spec, zb
                )
            else:
                self.d_print("RadialVelocityAlg: all wavelength zero")
        result_ccf[~np.isfinite(result_ccf)] = 0.0
        return result_ccf, ""
    ...
    def get_fiber_key_in_maskline(self):
        key_in_maskline = self.get_fiber_object_in_header(
            self.spectro, self.orderletname
        )
    ...
    def get_orderlet_masktype(ins, orderletname, rv_init_data):
        if not rv_init_data[
            RadialVelocityAlgInit.MASK_ORDERLET
        ]:  # no need ins and orderletname
            ...
            return rv_init_data[RadialVelocityAlgInit.MASK_ORDERLET][fiber_key][
                RadialVelocityAlgInit.MASK_TYPE
            ]
    ...
    def get_orderlet_maskpath(ins, orderletname, rv_init_data):
        if not rv_init_data[
            ...
            fiber_key = RadialVelocityAlg.get_fiber_object_in_header(ins, orderletname)
            return rv_init_data[RadialVelocityAlgInit.MASK_ORDERLET][fiber_key][
                RadialVelocityAlgInit.MASK_PATH
            ]
    ...
    def cross_correlate_by_mask_shift(self, wave_cal, spectrum, vb):
        ...
        # from the original
        line_index = np.where(
            (line.get("bc_corr_start") > np.min(wave_cal))
            & (line.get("bc_corr_end") < np.max(wave_cal))
        )[0]
        ...
        x_pixel_wave_start = (
            wave_cal + np.roll(wave_cal, 1)
        ) / 2.0  # w[0]-(w[1]-w[0])/2, (w[0]+w[1])/2.....
        x_pixel_wave_end = np.roll(
            x_pixel_wave_start, -1
        )  # (w[0]+w[1])/2,      (w[1]+w[2])/2....
    ...
    def calc_ccf(
        ...
        for c in range(v_steps):
            ...
            closest_match = np.sum(
                (x_pixel_wave_end - line_doppler_shifted_start[:, np.newaxis] < 0.0),
                axis=1,
            )
            closest_match_next = np.sum(
                (x_pixel_wave_start - line_doppler_shifted_end[:, np.newaxis] <= 0.0),
                axis=1,
            )
            ...
            idx_collection = list()
            for k in range(n_line_index):
                closest_x_pixel = closest_match[
                    k
                ]  # closest index starting before line_dopplershifted_start
                closest_x_pixel_next = closest_match_next[
                    k
                ]  # closest index starting after line_dopplershifted_end
                line_start_wave = line_doppler_shifted_start[k]
                line_end_wave = line_doppler_shifted_end[k]
                line_weight = new_line_weight[k]
                ...
                if closest_x_pixel_next <= pix1 or closest_x_pixel >= pix2:
                    ...
                    for n in range(closest_x_pixel, closest_x_pixel_next):
                        ...
                        # if there is overlap
                        if (
                            x_pixel_wave_start[n] <= line_end_wave
                            and x_pixel_wave_end[n] >= line_start_wave
                        ):
                            wave_start = max(x_pixel_wave_start[n], line_start_wave)
                            wave_end = min(x_pixel_wave_end[n], line_end_wave)
                            mask_spectra_doppler_shifted[n] = (
                                line_weight
                                * (wave_end - wave_start)
                                / (x_pixel_wave_end[n] - x_pixel_wave_start[n])
                            )
    ...
    def analyze_ccf(self, ccf, row_for_analysis=None):
        ...
        # skip order 0
        ccf[total_seg_rv + self.ROWS_FOR_ANALYSIS - 1, :] = np.nansum(
            ccf[row_for_analysis, :], axis=0
        )
        return ccf
    ...
    def get_rv_estimation(hdu_header, init_data=None):
        ...
        if "QRV" in hdu_header:
            ...
            rv_guess = init_data[RadialVelocityAlgInit.RV_CONFIG][
                RadialVelocityAlgInit.STAR_RV
            ]
    ...
    def rv_estimation_from_ccf_order(ccf_v, velocities, first_guess, mask_method=None):
        ...
        ccf_dir = (
            1
            if (mask_method is not None) and (mask_method in ["thar", "lfc", "etalon"])
            else -1
        )
        ...
        if abs_min_idx == abs_max_idx:  # no ccf result (all zero)
            ...
            vel_order = velocities[
                abs_min_idx
            ]  # if first_guess != 0.0 else first_guess
            ccf_order = ccf_v[abs_min_idx]
    ...
    def fit_ccf(
        ...
        if rv_guess_on_ccf:  # kpf get rv_guess from the ccf values
            ...
            rv_guess, ccf_guess, ccf_dir = (
                RadialVelocityAlg.rv_estimation_from_ccf_order(
                    result_ccf, velocities, rv_guess, mask_method
                )
            )
        ...
        def gaussian_rv(v_cut, rv_mean, sd):
            ...
            amp = (
                max(-1e7, np.nanmin(g_y) - y_dist)
                if ccf_dir < 0
                else min(1e7, np.nanmax(g_y) + y_dist)
            )
        ...
        if g_fit is not None and g_x[0] <= g_fit.mean.value <= g_x[-1] and two_fitting:
            v_cut = 25.0
        ...
        if vel_span_pixel is not None and vel_span_pixel != 0.0 and g_fit is not None:
            ...
            rv_error = RadialVelocityAlg.ccf_error_calc(
                velocities, result_ccf, f_wid * 2, vel_span_pixel, g_mean
            )
        ...
        if g_fit2 is not None and not math.isnan(g_fit2.mean.value):
            ...
            return (
                g_fit,
                (0.0 if math.isnan(g_fit.mean.value) else g_fit.mean.value),
                g_x,
                g_y,
                rv_error,
            )
    ...
    def compute_segments_ccf(self, ccf, analysis_rows=False, pixel_span=None):
        total_segments = (
            np.shape(ccf)[0] - RadialVelocityAlg.ROWS_FOR_ANALYSIS
            if analysis_rows
            else np.shape(ccf)[0]
        )
        ...
        v_span = pixel_span
        mask_type = self.get_orderlet_masktype(
            self.spectro, self.orderletname, self.init_data
        )
        for i in range(total_segments):
            _, rv_segments[i], _, _, erv_segments[i] = self.fit_ccf(
                ccf[i, :],
                self.get_rv_guess(),
                self.init_data[RadialVelocityAlgInit.VELOCITY_LOOP],
                mask_type,
    ...
    def output_ccf_to_dataframe(
        self, ccf, ref_head=None, original_ccf=None, ccf_rw_nonnorm=None, ref_ccf=None
    ):
        ...
        if (
            ref_ccf is not None and ccf_rw_nonnorm is not None
        ):  # use original ccf to get rv error
            _, _, _, _, rv_error = self.fit_ccf(
                ccf_rw_nonnorm[-1, :],
                self.get_rv_guess(),
                self.init_data[RadialVelocityAlgInit.VELOCITY_LOOP],
                self.get_orderlet_masktype(
                    self.spectro, self.orderletname, self.init_data
                ),
                rv_guess_on_ccf=(self.spectro == "kpf"),
        ...
        rv_segments, erv_segments = self.compute_segments_ccf(
            ccf,
            analysis_rows=True,
            pixel_span=self.get_vel_span_pixel() if ref_ccf is None else None,
        )
        if ref_ccf is not None and ccf_rw_nonnorm is not None:
            _, erv_segments = self.compute_segments_ccf(
                ccf_rw_nonnorm, analysis_rows=True, pixel_span=self.get_vel_span_pixel()
            )
        ...
        if self.spectro == "harps":
                    ...
                    if key in results.attrs or key == "COMMENT":
                        ...
                        if key == "Date":
                            ...
                            if "ESO DRS CCF RVC" in key:
                                results.attrs["CCF-RVC"] = (
                                    f_decimal(rv_result) + " Baryc RV (km/s)"
                                )
                        else:
            ...
            results.attrs["CCFJDSUM"] = self.get_obs_time(
                seg=-1
            )  # exposure time from the header
            results.attrs["CCFJDSEG"] = self.obs_jd  # an array for all segments

            obs_bjd = (
                np.zeros_like(self.obs_jd) if self.obs_bjd is None else self.obs_bjd
            )
            if obs_bjd[0] == 0.0:
                bc_config = self.configure()
                for i in range(obs_bjd.size):
                    obs_bjd[i], _, _ = BarycentricCorrectionAlg.get_bjd(
                        bc_config, self.obs_jd[i]
                    )
            ...
            results.attrs["CCF-RVC"] = (
                f_decimal(rv_result),
                "BaryC RV (km/s)",
            )  # rv based on CCF summation
            results.attrs["CCFSTART"] = self.init_data[
                RadialVelocityAlgInit.VELOCITY_LOOP
            ][0]
            ...
            results.attrs["BARY"] = (
                self.zb * 1.0e-3
            )  # from m/sec to km/sec, an array for all segments
            results.attrs["STARRV"] = self.init_data[RadialVelocityAlgInit.RV_CONFIG][
                RadialVelocityAlgInit.STAR_RV
            ]
            ...
            results.attrs["RV_MEAN"] = self.weighted_rv(
                rv_segments, rv_segments.size, ratio_ccf
            )  # weighted rv
            results.attrs["ERV_MEAN"] = self.weighted_rv_error(
                erv_segments, erv_segments.size, ratio_ccf
            )  # weighted rv error
            results.attrs["ORI_CCF"] = original_ccf if ref_ccf is not None else None
            results.attrs["CCF_NONNORM"] = (
                ccf_rw_nonnorm if ccf_rw_nonnorm is not None else original_ccf
            )
    ...
    def compute_rv_by_cc(
        self, start_seg=None, end_seg=None, ref_ccf=None, print_progress=None
    ):
        ...
        if self.spectrum_data is None or self.spectrum_data.size == 0:
            return {
                ...
                "ccf_ary": None,
                "jd": self.obs_jd,
                "msg": "no spectral data",
            }
        elif self.is_none_fiberobject(
            self.get_fiber_object_in_header(self.spectro, self.orderletname)
        ):
            return {
                "ccf_df": None,
                "ccf_ary": None,
                "jd": self.obs_jd,
                "msg": "fiber object is None",
            }
        ...
        if ref_ccf is not None:
            analyzed_rw_ccf, _ = self.reweight_ccf(
                ccf[0:total_seg_rv, :],
                end_seg - start_seg + 1,
                ref_ccf,
                self.reweighting_ccf_method,
            ...
            analyzed_rw_ccf_nonnorm, _ = self.reweight_ccf(
                ccf[0:total_seg_rv, :],
                end_seg - start_seg + 1,
                ref_ccf,
                self.reweighting_ccf_method,
        ...
        df = self.output_ccf_to_dataframe(
            (analyzed_ccf if analyzed_rw_ccf is None else analyzed_rw_ccf),
    ...
    def ccf_error_calc(velocities, ccfs, fit_wid, vel_span_pixel, rv_guess=0.0):
        ...
        n_scale_pix = (
            vel_step / vel_span_pixel
        )  # number of spectral pixels per ccf velocity step

        inds_fit = np.where(
            (velocities >= (rv_guess - fit_wid / 2.0))
            & (velocities <= (rv_guess + fit_wid / 2.0))
        )
    ...
    def is_good_reweighting_method(method, for_ratio=False):
        return (
            method in ["ccf_max", "ccf_mean"]
            if for_ratio
            else method in RadialVelocityAlg.CCF_Methods
        )

    @staticmethod
    def make_reweighting_ratio_table(
        order_val, s_idx, e_idx, reweighting_method, max_ratio=1.0, output_csv=""
    ):
        ...
        """
        if not RadialVelocityAlg.is_good_reweighting_method(
            reweighting_method, for_ratio=True
        ):
        ...
        ratio_table = {
            "segment": np.arange(0, t_seg, dtype=int),
    ...
    def weighted_rv(rv_arr, total_segment, reweighting_table_or_ccf, s_seg=0):
        ...
        if np.shape(reweighting_table_or_ccf)[1] >= 2:
            ...
            c_idx = np.where(
                (reweighting_table_or_ccf[:, 0] >= s_seg)
                & (reweighting_table_or_ccf[:, 0] <= e_seg)
            )[0]
            tval = reweighting_table_or_ccf[
                c_idx, -1
            ]  # selected weighting on selected segment
            sval = reweighting_table_or_ccf[c_idx, 0].astype(
                int
            )  # selected original segment index
        else:
    ...
    def weighted_rv_error(sigma_arr, total_segment, reweighting_table_or_ccf, s_seg=0):
        ...
        r_sigma_arr = [
            tval[s] / sigma_arr2[s] ** 2 if sigma_arr2[s] != 0.0 else 0.0
            for s in range(np.shape(sigma_arr2)[0])
        ]
    ...
    def reweight_ccf(
        ...
        if not reweighting_method or not RadialVelocityAlg.is_good_reweighting_method(
            reweighting_method
        ):
        ...
        if (
            ...
            or reweighting_method == "ccf_mean"
            or reweighting_method == "ccf_static"
        ):
            ...
            if reweighting_method == "ccf_static":
                ccf_sums = (
                    np.nansum(crt_rv, axis=1) if normalized else np.ones(total_segment)
                )  # summation along each order

                for idx in range(total_segment):
                    if ccf_sums[idx] > 0.0 and tval[idx] != 0.0:
                        new_crt_rv[sval[idx], :] = (
                            crt_rv[idx, :] / ccf_sums[idx]
                        ) * tval[idx]
            else:
                max_index = np.where(tval == np.max(tval))[0][
                    0
                ]  # the max from ratio table, 1.0 if ratio max is 1.0
                oval = (
                    np.nanpercentile(crt_rv[0:total_segment], 95, axis=1)
                    if reweighting_method == "ccf_max"
                    else np.nanmean(crt_rv[0:total_segment], axis=1)
                )  # max or mean from each order

                oval_at_index = oval[max_index]  # value from oder of max_index
                if (
                    oval_at_index == 0.0
                ):  # order of max_index has value 0.0, skip reweighting, returns all zeros
                    return new_crt_rv
                oval = (
                    oval / oval_at_index
                )  # ratio of orders before reweighting, value at order of max_index is 1.0

                for order in range(total_segment):
                    if oval[order] != 0.0:
                        new_crt_rv[sval[order], :] = (
                            crt_rv[order, :] * tval[order] / oval[order]
                        )
        elif (
            reweighting_method == "ccf_steps"
        ):  # assume crt_rv and reweighting ccf cover the same orders
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", category=RuntimeWarning)
                new_crt_rv = np.zeros(
                    (total_segment + RadialVelocityAlg.ROWS_FOR_ANALYSIS, nx)
                )
                for order in range(total_segment):
                    if np.size(np.where(crt_rv[order, :] != 0.0)[0]) > 0:
                        new_crt_rv[order, :] = crt_rv[order, :] * np.nanmean(
                            reweighting_table_or_ccf[order, :] / crt_rv[order, :]
                        )

        if do_analysis:
            row_for_analysis = np.arange(0, ny, dtype=int)
            new_crt_rv[ny + RadialVelocityAlg.ROWS_FOR_ANALYSIS - 1, :] = np.nansum(
                new_crt_rv[row_for_analysis, :], axis=0
            )
