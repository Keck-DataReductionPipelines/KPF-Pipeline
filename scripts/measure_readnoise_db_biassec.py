#################################################################################################
# measure_readnoise_db_biassec.py        Oct 18, 2023        Russ Laher (laher@ipac.caltech.edu)
#
# Directly measure read noise in KPF frames of any IMTYPE for all read-out channels in
# GREEN and RED chips, using the difference-image technique, on BIASSEC image regions
# (meaning overscan regions).  Thus, this code is NOT restricted to bias frames.
#
...
#imtype = 'Dark'
#query_template = "select mjdobs,filename,checksum from l0files where imtype='IMTYPE' and dateobs > '2022-12-01' order by mjdobs;"
#outfilename = 'read_noise_biassec_dark_1200.txt'

n_sigma = 2.0    # Number of sigmas for outlier rejection.
imtype = 'Flatlamp'
query_template = "select mjdobs,filename,checksum from l0files where imtype='IMTYPE' and exptime = 30 and dateobs > '2022-12-01' order by mjdobs;"
outfilename = 'read_noise_biassec_flat_30.txt'
...
def compute_read_noise(ext,fname1_input,fname2_input,n_sigma,cf):
    ...
    #
    #    Remember, the above indexes are in the FORTRAN representation!
    #
...
def main():
    ...
    query = query_template.replace("IMTYPE",imtype)
    ...
    if n % 2 == 1:             # Ensure pairs of frames to difference.
        n = n - 1
