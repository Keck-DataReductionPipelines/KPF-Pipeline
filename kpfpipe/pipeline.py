...
class Pipeline(object):
    ...
    # define enter and exit methods for 'with'
    ...
    def __str__(self):
        ...
        s = "Pipeline Object:\n"
        s += 'Methods completed:\n'
        s += '\n'.join(self.method_list)

        return s

    @classmethod
    def record_receipt(self, method_function):
        def method_wrapper(self, *args, **kwargs):
            logging.info('Running method %s' % str(method_function))
            logging.debug('Appending method to method list')
            arglist = [a.__str__() for a in args]
            arglist = ','.join(arglist)
            kwlist = ["{}={}".format(key, val.__str__()) for key, val in kwargs.items()]
            kwlist = ','.join(kwlist)
            if (len(arglist) + len(kwlist)) > 0:
                argstr = ','.join([arglist, kwlist])
                if argstr.startswith(','):
                    argstr = argstr[1:]
            else:
                argstr = ""
            mstr = str(method_function.__name__) + "({})".format(argstr)
            self.method_list.append(mstr)
            logging.debug('Executing method {}'.format(mstr))
            method_function(self, *args, **kwargs)
            self.checklevel(0)
    ...
    def to_fits(self):
        pass
        # write out current state of pipe object
    ...
    def save_receipt(self, outfile):
        ...
        with open(receiptfile, 'w') as of:
            print("Methods run as of UTC {}".format(datetime.utcnow()), file=of)
            for m in self.method_list:
                print(m, file=of)

        print("Receipt saved to {}".format(receiptfile))

#######


class KPFPipe(Pipeline):
    """
    Inherit from the Pipeline class and write instrument-specific primitives
    """
    ...
    #   This decorator will be applied to every level 0 method. This is superceeded by the Pipeline.record_receipt
    #   decorator, but we should figure out how to combine this with data validation checks as well.
    #   It will:
    ...
    # These are all level 0 methods, which can be called in any order as long as there is a level0 object in this
    # Pipeline instance Each level0 method must be tagged with the @level0_method decorator

    @Pipeline.record_receipt
    ...
    @Pipeline.record_receipt
    def extract_spectrum(self, chips=True, orders=[1,2,3]):
        # orders argument just for testing receipt recording
