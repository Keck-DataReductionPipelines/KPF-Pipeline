...
**Junk-frame exclusion.** "Junk" is a manual flag observers set at exposure time (e.g. wrong telescope settings): such a frame can pass every automated QC yet be scientifically useless. The authoritative list is WMKO's `{KPF_DATA_INPUT}/reference/Junk_Observations_for_KPF.csv` — a data-tree artifact, *not* a repo file (title line, `observation_id` header, one obs_id/row). `utils/io.py::load_junk_obs_ids(data_input)` is the single reader (absent file ⇒ empty set ⇒ no-op). It feeds two paths: (1) `build_mini_database` tags each frame with a derived **`ISJUNK`** column (frames are flagged, never dropped), which `build_l0_file_lists(exclude_junk=True)` filters out before master stacking and `rv_timeseries.py` uses to skip junk during discovery; (2) `QCL0.not_junk` populates the `NOTJUNK` QC flag on science frames, recovering `KPF_DATA_INPUT` from the L0's `self.dirname` (`{KPF_DATA_INPUT}/L0/{datecode}`, set by rvdata's `from_fits`). A cached mini-database CSV predating `ISJUNK` makes `build_l0_file_lists` **fail loudly** (`KeyError: 'ISJUNK'`) — regenerate it (the CSVs are gitignored build artifacts).
