...
# Copy the Python-script log file from runtime directory to product directory.

if (-e $pylogfile) {

    if (! (copy($pylogfile, $destdir))) {
        print "*** Warning: couldn't copy $pylogfile to $destdir ($!); " .
            "skipping...\n";
    } else {
        print "Copied $pylogfile to $destdir\n";
    }
}


# Copy the log file generated by this Perl script to product directory, assuming
