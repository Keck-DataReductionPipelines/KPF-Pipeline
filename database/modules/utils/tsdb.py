...
class TSDB:
    ...
    Arguments:
        backend (string) - database to use ('sqlite' or 'postgres')
    ...
    def __init__(self, backend='sqlite', db_path='kpf_ts.db', base_dir='/data/L0', logger=None, verbose=False):
        ...
        # Can these be generated by reading the metadata table?
        ...
        # If needed, create metadata table
    ...
    def ingest_one_observation(self, dir_path, L0_filename):
        ...
        if not self._is_any_file_updated(L0_file_path):
            return
        ...
        header_data.update({
            ...
            'Source': self.get_source(header_data),
            'L0_filename': L0_filename,
...
#            print("Keyword-to-table mapping:", kw_to_table)
            ...
            # Explicitly convert integer-like boolean columns to True/False
            for tbl, data in table_data.items():
                for kw in data:
                    if kw in self.bool_columns:
                        data[kw] = (str(data[kw]).strip().lower() in ['1', 'true', 't', 'yes', 'y'])

            # Ensure ObsID is always populated (excluding metadata table):
            for tbl in self.tables:
                if tbl != 'tsdb_metadata':
                    table_data.setdefault(tbl, {})['ObsID'] = base_filename

            # Diagnostic log for empty data tables
            for tbl, data in table_data.items():
                if len(data) <= 1:  # only ObsID or nothing else
                    self.logger.warning(f"Insufficient data for table {tbl}. Verify metadata and extraction logic.")
    ...
    def get_keyword_types(self, level):
        """
        Returns a dictionary of the data types for keywords at the 
        L0/2D/L1/L2/L0_telemetry/L2_RV_header/L2_CCF_header/L2_RV/base level.
        """
...
def process_file(file_path, now_str,
                 ...
                 #_is_any_file_updated_func, # this check was moved
