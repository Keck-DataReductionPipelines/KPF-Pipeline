...
The code for TSDB ingestion is described here: :doc:`../tsdb/analyzetimeseries`.
During production processing, data are ingested into the TSDB in the three ways described below. 
Note that 'ingestion' is a process that retrieves data from all available L0/2D/L1/L2 files for the ObsID 
...
In the first ingestion method, the script ``scripts/ingest_watch_kpf_tsdb.py`` has methods for watching and for period ingestion.  
...
The second ingestion method is the 'periodic' thread of ``scripts/ingest_watch_kpf_tsdb.py`` which 
scans the data directories listed above every 48 hours with ``force_ingest=True``.  
This catches any files that were missed for whatever reason by the watcher threads.

The third ingestion method happens during reprocessing with the ``scripts/reprocess_obs.py`` script.
As the script iterates over datecodes (e.g., `20251201`), observations 
